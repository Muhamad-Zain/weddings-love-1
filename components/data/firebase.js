...
const fetchData = async (id) => {
    ...
    try {
        ...
        const data = await get(child(dbRef, `/weddings-love-1/${id}`))
        if (data.exists()) {
            return data.val()
        } else {
...
const fetchBg = async (id) => {
    try {
        const galleryRef = storageRef(storage, `weddings-love-1/${id}`); // Referensi ke folder '01/galery'
...
const fetchGalery = async (id) => {
    try {
        const galleryRef = storageRef(storage, `weddings-love-1/${id}/galery`); // Referensi ke folder '01/galery'
...
const addDataToFirebase = async (id, formData) => {
    ...
    try {
      await set(ref(database, `/weddings-love-1/${id}`), formData);
